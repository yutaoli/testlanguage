...
total_income_aver_range_count=sqldf("select total_income_aver_range, count(*) as line_count,sum(swgt_p) as count from hh_income_with_weight_w group by total_income_aver_range")
total_percentage<-total_income_aver_range_count$count/sum(total_income_aver_range_count$count)#占比
total_income_aver_range_count<-cbind(total_income_aver_range_count,total_percentage)
